...
# 进一步封装Dataset，供读取训练用的<bug, patch>对
...
def semantic_training(generator, gen_opt, gen_tokenizer, adv_loader, device, epoch):
    ...
    for _, data in enumerate(adv_loader, 0):
        ...
        y_ids = y[:, :-1].contiguous() # 作为decoder的输入，训练阶段decoder的输入使用的是Y(ground truth)
        lm_labels = y[:, 1:].clone().detach() # 创建了一个与目标序列y相同的新张量lm_labels，并将其与y解耦，以确保对lm_labels的修改不会影响到y
        lm_labels[y[:, 1:] == gen_tokenizer.pad_token_id] = -100 # 忽略padding的token
        ...
        bugcode = bugcode[3 : end_index-1] # your index may be different! 剥离bug和context，因为其数据格式bug和context都是写在buggy下

        # 真实的buggy code和generator的输出outputs
...
# 句法训练的方法
def syntactic_training(epoch, tokenizer, model, device, loader, optimizer):
    model.train()
    countInt = 0
...
def syntactic(epoch,syn_train_data_path):
    ...
    # Process data
    df = pd.read_csv(syn_train_data_path,encoding='latin-1',delimiter=',', header=0, on_bad_lines='skip')
    # print(df.head())
    df = df[['bugid','buggy','patch']]
    print(df.head())
    ...
    syntactic_training(epoch, tokenizer, model, device, training_loader, optimizer)
    model.save_pretrained(SAVE_MODEL)
    tokenizer.save_pretrained(SAVE_MODEL)
    print(f'Syntatic Train Model Saved: {epoch}')

# 调用semantic_training，进行语义训练  
def semantic(epoch, semantic_train_data_path):   
...
if __name__ == '__main__':
    ...
    syn_train_data_path_1 = r'/home/lqy/Workspace/dataset/RewardRepair_data_2/training_data.csv'
    syn_train_data_path_2= './data/MegaDiff-CodRep.csv'
    semantic_train_data_path = r'/home/lqy/Workspace/rewardrepair/Bears_Training/BearsTraining.csv'
    SAVE_MODEL ='./model/RewardRepair_new'
    rootPath ='/home/lqy/Workspace'
    TRAIN_BATCH_SIZE = 20   
    ...
    MAX_LEN = 512
    PATCH_LEN = 100     
    # We train on our dataset without dataleakage and with semantic transformation to prevent dataleakage and enhance the robustness.
