...
if __name__=="__main__":
    path = r'/home/lqy/Workspace/coconut/Coconut_results'
    bug_name_id_list = os.listdir(path)
    bug_name_id_list.sort()
    for bug_name_id in bug_name_id_list[220:]:
        trans_patch_dir = os.path.join(os.path.join(path, bug_name_id))
        bug_name = bug_name_id.split('---')[0]
        bug_id = bug_name_id.split('---')[1]
        buggy_project = bug_name + '_' + bug_id
        # print(buggy_project)
        bug_project_dir = os.path.join(r'/home/lqy/Workspace/rewardrepair/Coconut_Defects4J_projects', buggy_project)
        ...
        if project_sub_path == '' or ',' in project_sub_path:
            print(buggy_project)
            continue
        bug_project_file = os.path.join(bug_project_dir, project_sub_path)
        validate_patches(trans_patch_dir, bug_project_dir, bug_project_file)